...
export function FornaContainer(element, passedOptions = {}) {
  ...
  var vis = mouseEventHelper.append('svg:g').classed(fstyle.plot, true);
  ...
  let toggleSelectNode = (node) => {
    ...
    visNodes
      ...
      .classed(fstyle.selectedNode, function (p) {
        return p.selected;
      });
  };
  ...
  let deselectAllNodes = () => {
    visNodes.selectAll('g.gnode').classed(fstyle.selectedNode, function (p) {
  ...
  let createNewNodes = function (gnodesEnter) {
    gnodesEnter = gnodesEnter
      .append('g')
      .classed('gnode', true)
      .attr('struct_name', function (d) {
        return d.structName;
      })
    ...
    // nucleotide labels
    var labelsEnter = gnodesEnter
      .append('text')
      .text(function (d) {
        return d.name;
      })
      .attr('class', fstyle.nodeLabel)
      .attr('label_type', function (d) {
        return d.nodeType;
      });
  ...
  let createNewLinks = function (linksEnter) {
    ...
    linkLines
      ...
      .attr('pointer-events', function (d) {
        if (d.linkType == 'fake') return 'none';
        else return 'all';
      });
  ...
  function drawDirectionArrow(d) {
    ...
    let path =
      'M' +
      (arrowTip[0] + lengthMult * (u[0] / 2 + (v[0] * arrowWidth) / 2)) +
      ',' +
      (arrowTip[1] + lengthMult * (u[1] / 2 + (v[1] * arrowWidth) / 2)) +
      'L' +
      arrowTip[0] +
      ',' +
      arrowTip[1] +
    ...
    for (let i = 0; i < self.extraLinks.length; i++) {
      ...
      if (self.extraLinks[i].linkType == 'intermolecule') {
        ...
        let fakeLinks = self.graph.links.filter(function (d) {
          return (
            (d.source == self.extraLinks[i].source ||
              d.source == self.extraLinks[i].target ||
              d.target == self.extraLinks[i].source ||
              d.target == self.extraLinks[i].source) &&
            d.linkType == 'fake'
          );
        });
  ...
  self.update = function () {
    ...
    let gnodes = visNodes.selectAll('g.gnode').data(self.graph.nodes, elementKey);
    ...
    let realNodes = self.graph.nodes.filter(function (d) {
    ...
    self.force.on('end', () => {
      gnodes
        .selectAll('[node_type=nucleotide]')
        .filter((d, i) => {
          if (i == 0) return true;
          else return false;
        })
  ...
  self.transitionRNA = function (newStructure, nextFunction) {
    ...
    if (duration === 0) gnodes.exit().remove();
    else
      gnodes
        .exit()
        .transition()
  ...
  let getBoundingBoxTransform = () => {
    ...
    var minX = d3.min(
      self.graph.nodes.map(function (d) {
        return d.x;
      }),
    );
    var minY = d3.min(
  ...
  self.changeColorScheme = function (newColorScheme) {
    ...
    proteinNodes.classed(fstyle.node, true).attr('r', function (d) {
      return d.radius;
    });
    ...
    if (newColorScheme == 'sequence') {
      ...
      if (
        typeof self.customColors != 'undefined' &&
      ...
      nodes.style('fill', function (d) {
        ...
        if (
          self.customColors.colorValues.hasOwnProperty(d.structName) &&
  ...
  let updateStyle = () => {
    // Numbering
    visNodes
      .selectAll('[node_type=label]')
      .classed(fstyle.transparent, !self.displayParameters.displayNumbering);
    visNodes
      .selectAll('[label_type=label]')
      .classed(fstyle.transparent, !self.displayParameters.displayNumbering);
    visLinks
      .selectAll('[link_type=label_link]')
      .classed(fstyle.transparent, !self.displayParameters.displayNumbering);
    // Node Outline
    svg
      .selectAll('[node_type=nucleotide]')
      .classed(fstyle.transparent, !self.displayParameters.displayNodeOutline);
    // Node Labels
    visNodes
      .selectAll('[label_type=nucleotide]')
      .classed(fstyle.transparent, !self.displayParameters.displayNodeLabel);
    // Links
    svg
      .selectAll(
        '[link_type=real],[link_type=basepair],[link_type=backbone],[link_type=pseudoknot],[link_type=protein_chain],[link_type=chain_chain],[link_type=external]',
      )
      .classed(fstyle.transparent, !self.displayParameters.displayLinks);
    // Pseudoknot Links
    svg
      .selectAll('[link_type=pseudoknot]')
      .classed(fstyle.transparent, !self.displayParameters.displayPseudoknotLinks);
    // Protein Links
    svg
      .selectAll('[link_type=protein_chain]')
      .classed(fstyle.transparent, !self.displayParameters.displayProteinLinks);
    // Fake Links
    visLinks
      .selectAll('[link_type=fake]')
      .classed(fstyle.transparent, !self.options.displayAllLinks);
    visLinks
      .selectAll('[link_type=fake_fake]')
      .classed(fstyle.transparent, !self.options.displayAllLinks);
    // direction Arrows
    svg
      .selectAll('.' + fstyle.directionArrow)
      .classed(fstyle.transparent, !self.displayParameters.displayDirectionArrows);
  };
  ...
  let changeNode = function (nodeName, referenceNode) {
    ...
    let newRNA = self.addRNA(newDotbracket, {
  ...
  let deleteNode = function (node) {
    ...
    let newPositions = positions.slice(0, node.num - 1).concat(positions.slice(node.num));
    let newSequence = sequence.slice(0, node.num - 1) + sequence.slice(node.num);
    let newUids = uids.slice(0, node.num - 1).concat(uids.slice(node.num));
  ...
  let removeBackBoneLink = function (d) {
    ...
    for (let i = 0; i < rna.links.length; i++) {
      ...
      if (link.linkType != 'basepair') continue;
    ...
    delete self.rnas[rna.uid];
    let rna1 = self.addRNA(dotBracket1, {
      sequence: sequence1,
      positions: positions1,
      uids: uids1,
    });
    let rna2 = self.addRNA(dotBracket2, {
      sequence: sequence2,
  ...
  let addBackBoneLink = function (newLink) {
    ...
    self.extraLinks = self.extraLinks.filter((e) => {
    ...
    else newRna = self.addRNA(newDotbracket, { sequence: newSeq, centerView: false });
  ...
  let nodeMouseup = function (d, i) {
    ...
    if (mousedownNode) {
      ...
      if (
        mouseupNode.nodeType == 'middle' ||
        mousedownNode.nodeType == 'middle' ||
        mouseupNode.nodeType == 'label' ||
        mousedownNode.nodeType == 'label'
      )
        return;
      ...
      for (let i = 0; i < self.graph.links.length; i++) {
        if (
          ...
          if (
            self.graph.links[i].linkType == 'basepair' ||
        ...
        if (
          (self.graph.links[i].source == mouseupNode &&
            self.graph.links[i].target == mousedownNode) ||
          (self.graph.links[i].source == mousedownNode && self.graph.links[i].target == mouseupNode)
        ) {
      ...
      if (newLink.source.rna != newLink.target.rna) {
        ...
        if (
          (newLink.source.num == 1 && newLink.target.num == newLink.target.rna.rnaLength) ||
  ...
  self.addRNA = function (structure, passedOptions = {}) {
    ...
    if ('centerPos' in passedOptions)
      self.addRNAJSON(rnaJson, { centerPos: passedOptions.centerPos, centerView: false });
    else if ('avoidOthers' in passedOptions)
      self.addRNAJSON(rnaJson, { avoidOthers: passedOptions.avoidOthers });
    else self.addRNAJSON(rnaJson, { centerView: passedOptions.centerView });

    return rnaJson;
  };

  self.addRNAJSON = function (
    rnaGraph,
  ...
  self.getStructuresDotBracket = function () {
    ...
    for (let uid in self.rnas) {
      ...
      for (let j = 0; j < rna.nodes.length; j++) {
        ...
        if (node.nodeType != 'nucleotide') continue;
    ...
    pairtable = [currIdx - 1];
    for (let i = 0; i < currIdx; i++) pairtable.push(0);
