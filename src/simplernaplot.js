export function simpleXyCoordinates(pair_table) {
  ...
  var loop_size = Array.apply(null, new Array(16 + Math.floor(len / 5))).map(
    Number.prototype.valueOf,
    0,
  );
  var stack_size = Array.apply(null, new Array(16 + Math.floor(len / 5))).map(
    Number.prototype.valueOf,
    0,
  );
     ...
     i-1 and j+1 are paired. */
  {
    ...
    var remember = Array.apply(null, new Array(3 + Math.floor((j - i) / 5) * 2)).map(
      Number.prototype.valueOf,
      0,
    );
