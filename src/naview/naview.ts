...
export class NAView {
  ...
  rlphead: Radloop | null;

  lencut: number;
  RADIUS_REDUCTION_FACTOR: number;
  ...
  private noIterationFailureYet: boolean;
  ...
  naview_xy_coordinates(pair_table: number[]) {
    const x: number[] = [];
    const y: number[] = [];
  ...
  read_in_bases(pair_table: number[]) {
    let i = 0;
    let npairs = 0;
  ...
  private depth(lp: Loop): number {
    ...
    for (let i = 0; lp.getConnection(i) != null; i++) {
      const conn = lp.getConnection(i) as Connection;
      d = this.depth(conn.getLoop());
      if (d >= 0) {
        if (++count == 1) {
          ret = d;
        } else if (ret > d) {
  ...
  traverse_loop(lp: Loop, anchor_connection: Connection | null) {
    let xs, ys, xe, ye, xn, yn, angleinc, r;
    let radius, xc, yc, xo, yo, astart, aend, a;
    let cp: any, cpnext: any, acp: any, cpprev: any;
    let i, j, n, ic;
    let da, maxang;
  ...
  determine_radius(lp: Loop, lencut: number) {
    ...
    let i,
      ...
      start,
      imindit = 0;
    let cp: any = new Connection(),
      cpnext: any = new Connection();
    ...
    do {
      ...
      if (mindit * radius < lencut) {
        (lp.getConnection(imindit) as Connection).setExtruded(true);
      }
  ...
  find_ic_middle(
    icstart: number,
    icend: number,
    anchor_connection: Connection | null,
    acp: Connection,
    lp: Loop,
  ) {
    let count, ret, ic, i;
  ...
  construct_extruded_segment(cp: Connection, cpnext: Connection) {
  ...
  construct_circle_segment(start: number, end: number) {
  ...
  find_center_for_arc(n: number, b: number) {
  ...
  generate_region(cp: Connection) {
    ...
    if (
      this.bases[cp.getStart()].getX() > this.ANUM - 100.0 ||
  ...
  minf2(x1: number, x2: number) {
    return x1 < x2 ? x1 : x2;
  }

  maxf2(x1: number, x2: number) {
    return x1 > x2 ? x1 : x2;
  }

  connected_connection(cp: Connection, cpnext: Connection) {
    if (cp.isExtruded()) {
      return true;
